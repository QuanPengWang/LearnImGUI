...
bool mainWindow::Run()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
...

        {
            ...
            bool open = false;
            ImGui::Begin(u8"测试一下中文 ！", nullptr, ImGuiWindowFlags_NoTitleBar);

            ImGui::Text(u8"这是文本框。");

            ImGui::SliderFloat(u8"float浮点数", &f, 0.0f, 1.0f);            // Edit 1 float using a slider from 0.0f to 1.0f
            ImGui::ColorEdit3("clear color", (float*)&clear_color); // Edit 3 floats representing a color

            if (ImGui::Button(u8"按 钮"))                            // Buttons return true when clicked (most widgets return true when edited/activated)
                counter++;
            ImGui::SameLine();
            ImGui::Text(u8"计数 = % d", counter);

            ImGui::Text(u8"应用程序的平均 %.3f ms/帧数 (%.1f FPS)", 
